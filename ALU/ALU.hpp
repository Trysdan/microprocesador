...
SC_MODULE(ALU) {
    ...
    SC_CTOR(ALU) {
        ...
        for(int i=0; i<4; i++) {
            arithUnit->a[i](a[i]); arithUnit->b[i](b[i]);
            logicUnit->a[i](a[i]); logicUnit->b[i](b[i]);
            compUnit->a[i](a[i]);  compUnit->b[i](b[i]);
        ...
        for(int i=0; i<4; i++) {
            ...
            mux->in[2][i](s_ground); mux->in[3][i](s_ground);
            mux->in[8][i](s_ground); mux->in[9][i](s_ground);
            mux->in[13][i](s_ground); mux->in[14][i](s_ground); mux->in[15][i](s_ground);
        }
