...
SC_MODULE(AdderSub4Bit) {
    ...
    SC_CTOR(AdderSub4Bit) {
        ...
        for(int i=0; i<4; i++) sensitive << b[i] << ctrl;

        SC_METHOD(compute_pg);
        for(int i=0; i<4; i++) sensitive << a[i] << b_real[i];

        SC_METHOD(compute_carry);
        sensitive << ctrl; 
        for(int i=0; i<4; i++) sensitive << p[i] << g[i];

        SC_METHOD(compute_sum);
        for(int i=0; i<4; i++) sensitive << p[i] << c_int[i];
    }
};
